...
// Writes any startup/runtime crash info to a log file in the user's
// AppData folder, since a packaged app has no visible terminal.
// On Windows this ends up at: %APPDATA%\<AppName>\backend-error.log
function logToFile(label, err) {
  try {
    ...
    // If even logging fails, fall back to console (won't be visible in
    // production, but avoids crashing here).
...
function createWindow() {
  ...
  win.webContents.on(
    ...
    (event, errorCode, errorDescription, validatedURL) => {
      ...
      logToFile(
        "Page failed to load",
        new Error(`${errorDescription} (code ${errorCode}) - URL: ${validatedURL}`)
      );
    }
  );

  // DevTools open automatically for now so you can see errors on the
  // broken PC too. Once everything is confirmed working, change this
  // back to: if (!app.isPackaged) { win.webContents.openDevTools(); }
  win.webContents.openDevTools();

  win.loadFile(path.join(studentDistDir, "index.html"));
}

// Catch any error that isn't caught elsewhere in the main process
// (e.g. thrown during backend startup outside the try/catch, or from
// something unrelated) and write it to the same log file.
